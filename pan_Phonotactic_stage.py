...
import numpy as np
#setting directory
#cd /Users/joncparamore/Library/Mobile Documents/com~apple~CloudDocs/Linguistics/002_Current_Projects/002 Dissertation/Qualifying Exam/Disparity_Bias_Code/
...
CONSTRAINTS = "IDnas IDFin(nas) Sprd-L *NasObs *NasG *NasV ID(nas)/_V(-nas) *VN ID(rd) *LowRd".split()
#Whether the constraints are faithfulness (f) or markedness (m), ties to whether they are biased high or low.  
CONTYPE="f f m m m m cf m f m".split()
...
#output is a list of violations for the ten constraints..e.g., [1, 2, 0, 0, etc.]
def vios(stem, suf, outpair):
    ...
    ####*NasV
    ##Assign a violation for every nasal vowel that appears on the surface.
    #define variable to hold number of violations
    StarNasV = 0
    #iterate over every segment in the SR
    for i in range(len(SR)):
        output_segment = SR[i]
        if output_segment in NASALV:
            StarNasV += 1
    #append number of violations of StarNasV to the violation vector, violations
    violations.append(StarNasV)
...
cfbias = 100
#Bound used to prevent constraint weights from getting below zero.
negative=-0.00000001
