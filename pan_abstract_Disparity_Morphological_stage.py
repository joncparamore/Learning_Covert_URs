...
#LEX
#manually generate potential URs and set their probabilities of being the correct UR equal
LEX = {
  0: {'': 1},
  1: {'ʋA': 1}, 
  2: {'sɑ': 0.5, 'sA': 0.5},  
...
phonotactic_weights = [51.37,  44.83,  92.83, 100.00, 99.48, 100.00, 100.00, 0.00, 100.00] # [51.32, 44.26, 92.92, 100.00, 99.44, 100.00, 100.00, 0.00, 100.00]
...
negative=-0.00000001
#sigma_likelihood is a scaling factor that, when increased, makes likelihood of data more important than reducing disparities.
sigma_likelihood = 1.75
#sigma2 is a constant used in the objective function to scale the penalty for disparities between UR-SR. Higher value means larger penalty
sigma2 = 10
...
####MOD(x)
def MODx(Disparities):
    return sum((disparity**2) for disparity in Disparities)
...
def objective(parameters):
    ...
    objective_value = 0
    #For each surface form (e.g., sIn) calculate the likelihood of the data with all potential URs
    for stemtag, suftag in inp_out:
        ...
        for stem in URprobs_Normalized[stemtag]:
            ...
            for suf in URprobs_Normalized[suftag]:
                ...
                if surface_form in prob_stem_suf_input_pair:
                    ...
                    #define the probability of the current UR
                    ...
                    likelihood += current_UR_probability * prob_surfaceform_wins
        ...
        if likelihood <= 1e-150:
            ...
            objective_value += 1e500  # Large penalty
        else:
            #add negative log likelihood to objective_value.
            objective_value += -log(likelihood)
    
    ####Implement MOD(x) in the objective function
    #for each surface form find the disparities between the SR and each potential UR
    Disparities = []
    #iterate over every surface form
    for stemtag, suftag in inp_out:
        #define the surface form
        surface_form = inp_out[(stemtag, suftag)]
        #iterate over each possible stem-suffix UR pair, given the current surface form, and calculate the number of disparities
        for stem in URprobs_Normalized[stemtag]:
            for suf in URprobs_Normalized[suftag]:
                #make each possible stem-suffix UR pair the input
                input = (f"{stem}{suf}")
                #initialize variable to keep track of disparities between input and output
                disparity = 0
                #define the probability of the current UR
                current_UR_probability = URprobs_Normalized[stemtag][stem] * URprobs_Normalized[suftag][suf]
                #iterate over every segment in the UR adding nasal and round feature disparities
                for i in range(len(input)):
                    input_segment = input[i]
                    output_segment = surface_form[i]
                    if input_segment != output_segment:
                        if ((input_segment in NASAL) and (output_segment not in NASAL)) or ((input_segment not in NASAL) and (output_segment in NASAL)):
                            disparity += 1 * current_UR_probability 
                        if ((input_segment in RDV) and (output_segment not in RDV)) or ((input_segment not in RDV) and (output_segment in RDV)):
                            disparity += 1 * current_UR_probability
    ...
    return objective_value + prior(WEIGHTS) + (sigma * priorl(UR_probabilities)) + (sigma2 * MODx(Disparities))
            ...
            for stem in URprobs_Normalized[stemtag]:
                for suf in URprobs_Normalized[suftag]:
                    ...
                    if surface_form in candy:
                        ...
                        Harmony_score_scaling_constant = 40
                        for candidate in candset:
    ...
    for i in range(len(tag_UR_pairs)):
        ...
        for tags in inp_out:
            ...
            if tag in tags:
                ...
                ####Implement MOD(x) in the gradient function
                #for each surface form find the disparities between the SR and each potential UR
                disparity = 0
                #iterate over each possible stem-suffix UR pair, given the current surface form, and calculate the number of disparities
                current_UR_probability = URprobs_Normalized[tag][UR]
                #iterate over every segment in the UR
                #print(f'{UR} and {surface_form}')
                for i in range(len(UR)):
                    input_segment = UR[i]
                    output_segment = surface_form[i]
                    if input_segment != output_segment:
        ...
        #a higher gradient means the probability of the current UR needs to decrease.
        #subtracting likelihood because a higher likelihood of the data should decrease the gradient and indicate that the UR is doing well
        #add probability of the current UR to gradient to prevent probabilities from going to extremes unless data calls for it.
        #add disparity to gradient so that a greater disparity for high UR probabilities results in a steeper positive gradient           
        gradi = (-liky)*sigma_likelihood + ((2 * current_UR_prob) * sigma) + ((disparity)*liky)
        grad.append(gradi)
...
def printprob(parameter):
    ...
    for i in range(len(tag_UR_pairs_list)):
        ...
        if tag in URfreqs:
            ...
            URfreqs[tag] = {UR: probability}
    URprobs_Normalized = pLEX(URfreqs)
    likylist = {}
    for stem_suf_pair in inp_out:
        prob = {}
		#print y
        stemtag = stem_suf_pair[0]
        suftag = stem_suf_pair[1] 
		#print LEX
        for stem in URprobs_Normalized[stemtag]:
            for suf in URprobs_Normalized[suftag]:
                current_UR_probability = URprobs_Normalized[stemtag][stem] * URprobs_Normalized[suftag][suf]
                #input = (f"{stem}{suf}")
                candidate_probs = probs((stem, suf), ew1)
                for potential_SR_with_crelations in candidate_probs:
                    potential_SR = potential_SR_with_crelations[0]
                    if potential_SR in prob:
                        prob[potential_SR] += current_UR_probability * candidate_probs[potential_SR_with_crelations]
                    else:
                        prob[potential_SR] = current_UR_probability * candidate_probs[potential_SR_with_crelations]
...
res=scipy.optimize.minimize(objective,np.asarray(gradient_params[0]), method='SLSQP',bounds=bigbound,jac=gradient,
                            options={'maxiter':20000,'ftol':1e-10, 'eps':1e-5, 'disp': True})
